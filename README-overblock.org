...
button does.  The first candidate of every glyph is a codicon — a
name that begins =nf-cod-= in nerd-icons — so that one family draws
one weight and one size.
...
=MArpogaus/overblock-pycell=, the repository they live in today.  melpazoid asks
GitHub whether that URL answers, so it cannot name a repository that
does not exist yet.  The move begins with one command:

#+begin_src shell
  sed -i 's|MArpogaus/overblock-pycell|MArpogaus/overblock|' overblock*.el test/overblock*-test.el
#+end_src

and the recipe in =.github/workflows/pre-commit.yml= takes the same
change to its =:repo=.

The other half of the move is one line in the notebook: =overblock-pycell.el=
