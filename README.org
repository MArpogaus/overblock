...
- A converter from Markdown to HTML for the markdown cells.  pycell
  ...
  =cmark-gfm=, in this order.  See =overblock-md-command=.
...
A converter that renders math itself leaves nothing to preview: pandoc
turns =$x^2$= into markup of its own, while =markdown_py= and =cmark=
pass the dollars through.  See =overblock-md-command=.
