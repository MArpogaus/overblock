...
Every animation here was recorded from a real Emacs session under
[[file:demo/init.el][=demo/init.el=]], which is the whole setup behind
them — a built-in theme, one font, these packages and nothing else.
[[file:demo/README.org][=demo/README.org=]] says how to record them
again.
...
| =overblock-md=     | markup to a propertized string, and a Markdown preview mode | =overblock=, latex-to-svg-backend       |
| =overblock-pydoc=  | Python doc strings rendered in place                        | =overblock-md=                          |
| =overblock-pycell= | inline results for =# %%= Python cells                      | =overblock-md=, code-cells              |
...
=overblock-edit-abort= end the edit, on =C-c C-c= and =C-c C-k= in
=overblock-edit-mode-map=, the keys of =org-edit-special=.
...
| =:region-at= | the region point is in, as =(BEG . END)=         |
| =:starts=   | a marker on the start of every region, in order   |
| =:redraw=   | draw the mode's own bars again                    |

The same plist carries the look of a result block: =:keymap=,
=:buttons=, =:fold=, =:header-face=, =:output-face=, =:lines= and
=:chars=.  The buttons and the two limits name an option rather than
hold a value, so one the reader has just customized is read when the
block is drawn.  =overblock-run-attach= makes a buffer a notebook with
such a plist, and =overblock-run-detach= takes it back.
...
The commands a reader presses on a result are here as well, the same
in every notebook: =overblock-run-toggle-output=, =-copy-output=,
=-discard-output=, =-clear-results=, =-above=, =-stop= and
=-interrupt=.  Each package keeps its own buttons, faces and limits, and
the commands that know its cells; what they share is every
line below them.
...
| =overblock-live-idle=       | 0.2     | the quiet before a live cycle renders again |
...
LaTeX fragments become preview images by way of
[[https://github.com/alberti42/latex-to-svg-backend][latex-to-svg-backend]],
which the package depends on.  A table is laid
...
| =overblock-md-html-batch-async= | convert many texts in one process       |
| =overblock-md-columns=          | the columns a window leaves a rendering |
| =overblock-md-program=          | the converter that is installed, or nil |
| =overblock-md-eglot-renderer=   | render a language server's documentation |
...
a preview image, capped by =overblock-image-height=.
[[https://github.com/alberti42/latex-to-svg-backend][latex-to-svg-backend]]
makes it: it compiles with =latex= and =dvisvgm= in a process of its
own, so nothing waits, and keeps one compile per equation however many
buffers ask for it.  The SVG carries neither the colour nor the size,
which are given to it when it is drawn — a theme change and a
=text-scale-adjust= cost no LaTeX at all.  A fragment still compiling
stands as its own text and is drawn when it arrives; one that fails
stays text, and the engine reports it with a link to the log LaTeX
left.
...
once you have stopped.  Mark a region and the renderings it reaches show
their source too, so what you see is what you copy or cut; they come
back when the mark goes.
...
*** Documentation from a language server

eglot shows what a language server answers for the symbol at point
through eldoc, and renders the server's Markdown with a Markdown mode's
font lock.  Font lock cannot lay out a table, draw a formula or paint a
fenced block in its own language; this renderer can, and eglot has the
hook for it:
...
The documentation then reads the same wherever eldoc shows it — the
echo area, the =*eldoc*= buffer, an eldoc-box child frame — with a pipe
table in columns, the signature painted as code and a formula as an
image.  =overblock-md-eldoc-width= is the width the text is filled to,
72 columns by default, since no window has the text when it is
rendered.  A rendering is kept, so the second hover over a symbol costs
no process.  A server that answers plain text is left to eglot, and
without a converter the Markdown stays as it came.
...
| =overblock-md-forget-failed-images=   | fetch the images again that could not be reached |
| =overblock-live-edit=                 | show the source of the region at point |
...
| =overblock-md-code-modes=    | eleven languages that do not spell their mode    | the mode that paints a fenced block; nil for none |
| =overblock-md-code=          | inherits =font-lock-constant-face=               | the face of inline code                    |
| =overblock-md-eldoc-width=   | 72                                               | the columns a server's documentation is filled to |

A fenced block that names its language — =```python= — is painted by
that language's major mode, as the editor paints the file:
=overblock-md-code-modes= maps the names that do not spell their mode
(=elisp=, =bash=, =r= …), and every other name is tried as
=LANGUAGE-mode= through =major-mode-remap-alist=.  Set it to nil and
every block wears =overblock-md-code=.
...
markup is rendered, and the code around them is untouched.  Click one
and its source comes back to be edited; move away and it reads as
documentation again.  Point moving into a rendered doc string changes
nothing, so the code around it is edited with the prose in view.
...
A doc string wears a bar: the glyph, its first line, and the buttons
of =overblock-pydoc-buttons= at the window's edge, under the bar's own
rule.  The rest of the prose stands below it, and a rule closes it; a
doc string of one line is all bar.  The first line rides the bar
because the bar is the one row the rendering adds: with the summary on
it the rendering has exactly as many rows as the doc string, so nothing
shares a line, and a doc string of one line and one of twenty read
alike.  A summary longer than the room is cut with an ellipsis.
...
| Option                     | Default                        | Meaning                                 |
|----------------------------+--------------------------------+-----------------------------------------|
| =overblock-pydoc-markup=   | =markdown=                     | the markup the doc strings are written in |
| =overblock-pydoc-command=  | a =pandoc= command per markup  | the shell command for a doc string      |
| =overblock-pydoc-modes=    | =rst-mode=, =markdown-mode=    | the major mode that reads one, per markup |
| =overblock-pydoc-buttons=  | one edit button                | the buttons on that bar                 |
| =overblock-pydoc-footer=   | =shadow= with an underline     | the face of the rule under a doc string |

=overblock-pydoc-markup= says what the doc strings of a buffer are
written in, and everything else follows it: which command of
=overblock-pydoc-command= renders them, and which mode of
=overblock-pydoc-modes= =overblock-pydoc-edit= opens the source in.
Markdown is the default, because that is what a numpy style doc string
carries under its section titles today, as mkdocstrings reads it: a
fenced block, a pipe table, bold.  The Markdown command keeps every
source line a line, so a parameter entry and its description stay
apart.  A project whose doc strings are reStructuredText for Sphinx
...
One doc string is one markup: given the other reader, a pipe table
runs together into a paragraph of pipes, and a =:class:`x`= role stands
in the prose as it is written.  Both mode entries are yours to change.
...
=make scroll= tests this — from Emacs 30 on.  Emacs 29 scrolls a block
taller than the window and then recenters, so the wheel goes up and
the window comes down a little: the fault is
=pixel-scroll-precision-scroll-up-page='s, which leaves point outside
the window, and Emacs 30 rewrote it to put point back before redisplay
sees it.  The emacs-29 branch never took that fix, and nothing this
package can do stands in for it.
...
| =□=    | interrupt this cell, and stop the pass |
...
=overblock-run-interrupt= as a click, which is what a notebook's stop
does: the cell is interrupted and the cells queued after it do not
start.  =overblock-run-stop= is the softer stop, from the keyboard: the
running cell runs to its end and only the queue is emptied.

A pass over several cells — =overblock-pycell-restart-and-run-all= or
=overblock-run-above= — scrolls each cell it starts to the top
...
block wears.  =overblock-run-pop-output= gives you that table live,
...
=overblock-run-interrupt= and =overblock-run-stop= work in such a
...
watching it.  Bind them in =overblock-run-pop-map=; the buffer is
read-only, so a plain key is free.

=overblock-run-pop-output= on a cell that is still running keeps
...
tag, and carries the buttons of =overblock-pycell-md-buttons=:
=✎= opens the cell in a buffer of its own, and the move pair follows.
No button shows the source, because a click on the rendering does: the
cell shows its source while point is in it, and is rendered again once
point has left — the same live cycle that keeps a Markdown file
rendered.
...
source of one cell, to edit it where it stands.
...
| =overblock-run-toggle-output=          | fold or unfold the result at point                                 |
| =overblock-run-copy-output=            | copy the result at point                                           |
| =overblock-run-pop-output=             | show the result in its own buffer, and follow a running cell there |
| =overblock-run-save-image=             | write the image of the result to a file                            |
| =overblock-run-discard-output=         | remove the result at point                                         |
| =overblock-run-clear-results=          | remove every result                                                |
| =overblock-run-interrupt=              | send a KeyboardInterrupt to the cell, and stop the pass            |
...
| =overblock-run-above=                  | run every cell above the one at point                              |
| =overblock-run-stop=                   | stop the run of the cells after this one                           |
...
| =overblock-pycell-render-buffer=       | render each markdown cell that is not rendered                     |
| =overblock-pycell-md-render-cell=      | render the markdown cell at point                                  |
| =overblock-pycell-md-raw=              | show one markdown cell as source, until point leaves it            |
...
| =overblock-run-pop-map=       | a result popped out into its own buffer |
| =overblock-run-result-map=    | inside a cell that shows a result       |
...
#+begin_src emacs-lisp
  ...
  (keymap-set overblock-pycell-mode-map "C-c C-q" #'overblock-run-stop)
...
| =overblock-pycell-md-buttons=       | three buttons                     | the buttons on the header of a markdown cell     |
...
Zero is a legal value for both limits, and means the same in each:
=max-lines= of zero shows every line of the output inline, and
=max-line-length= of zero shows a line whole.
...
#+begin_src emacs-lisp
  ...
  (setopt overblock-pycell-result-buttons
          '((copy ("◫" "≡") "Copy this result" overblock-run-copy-output lines)
...
again when the option is set that way, and so are the results when
=max-lines= or =max-line-length= is.
...
that any of it is R.  Under polymode the buffer stays in its host mode
while the mode is on — the chunks are still fontified and indented as
R, but point entering one no longer switches to an indirect buffer,
which carried the bars and the results along and drew them twice.
...
|--------+--------------------------------|
| =▾=    | fold or unfold the result      |
| =□=    | interrupt this chunk, and stop the pass |
| =◫=    | copy the result                |
| =✕=    | discard the result             |
...
A chunk that plots answers with its figure, drawn under what it
printed.  R draws to a graphics device and not to its terminal, so the
chunk runs with a PNG device open — at the =fig.width=, =fig.height=
and =dpi= its header writes, as under knitr, or at
=overblock-rmd-figure-size=, 7 by 5 inches, where it writes none — and
each page it drew comes back into the result.  From there a figure is
what it is in the Python notebook: capped to the window, saved with
=↧=, shown whole in a pop-out, and named in a terminal.
...
=overblock-run-above= and =overblock-rmd-restart-and-run-all= run
...
=overblock-run-stop= ends the pass after the chunk that is running now.
=overblock-run-interrupt= is the harder stop: R answers an interrupt
...
queued behind it, and point comes back when the queue is done.
...
| =overblock-rmd-run-chunk-and-step=  | run the chunk at point and move to the next               |
| =overblock-rmd-forward-chunk=       | move to the next chunk                                    |
| =overblock-rmd-backward-chunk=      | move to the previous chunk                                |
| =overblock-run-above=               | run every chunk above the one at point                    |
...
| =overblock-run-stop=                | stop the pass after the chunk that runs now               |
| =overblock-run-interrupt=           | interrupt the chunk R is running, and stop the pass       |
| =overblock-run-toggle-output=       | fold or unfold the result at point                        |
| =overblock-run-copy-output=         | copy the result at point                                  |
| =overblock-run-pop-output=          | show the result in its own buffer, and follow a running chunk there |
| =overblock-run-save-image=          | write the figure of the result to a file                  |
| =overblock-run-discard-output=      | remove the result at point                                |
| =overblock-run-clear-results=       | remove every result                                       |
...
=overblock-run-result-map= inside a chunk that shows a result, as it
does in a cell of the Python notebook; both are there, empty, so every
key is free to bind:

#+begin_src emacs-lisp
  (keymap-set overblock-rmd-mode-map "C-<return>" #'overblock-rmd-run-chunk)
  (keymap-set overblock-rmd-mode-map "S-<return>" #'overblock-rmd-run-chunk-and-step)
  (keymap-set overblock-rmd-mode-map "C-c C-k" #'overblock-run-interrupt)
  (keymap-set overblock-rmd-mode-map "C-c C-q" #'overblock-run-stop)
#+end_src
...
| =overblock-rmd-figure-size=     | =(7 . 5)=                 | width and height of a figure, in inches          |
| =overblock-rmd-chunk-buttons=   | two buttons               | the buttons on the bar of a chunk                |
| =overblock-rmd-result-buttons=  | five buttons              | the buttons on the header of a result            |
...
open Rmd buffer are drawn again when the option is set that way, and so
are the results when =max-lines= or =max-line-length= is.
...
- For anything that renders markup — =overblock-md= and the three
  ...
  - An Emacs with libxml2, because shr reads the converter's HTML with
    ...
    is.
  - =latex= and =dvisvgm= for the formulas the converter leaves alone,
    which is what
    [[https://github.com/alberti42/latex-to-svg-backend][latex-to-svg-backend]]
    runs; it arrives with the package.  A fragment whose preview fails
    stays text.
- =overblock-pydoc= also loads =rst=, which Emacs ships.
- =overblock-pycell= needs
  [[https://github.com/astoff/code-cells.el][code-cells]], which arrives
  as its dependency.  Text output needs nothing more.  Images and
  tables need [[https://github.com/astoff/comint-mime][comint-mime]],
  with =comint-mime-setup= on =inferior-python-mode-hook=, and an
  IPython: comint-mime installs its renderers through IPython, so a
  plain =python3= shell gives text only.
- =overblock-rmd= needs [[https://ess.r-project.org/][ESS]], which
  arrives as its dependency, and an R for ESS to start.
...
- A rendering makes each scroll event more expensive, because rendered
  ...
  runs and not from the height.  Turn the mode off to get the plain
  source and the speed back.
...
- A result block shows =overblock-pycell-max-lines=, or
  ...
  =overblock-run-pop-output= shows the whole of it, and keeps up
...
- One region runs at a time for each inferior process.  A cell or a
  chunk sent while another one runs is queued behind it.
