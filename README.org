...
A doc string of more than one line wears a bar above it — a glyph and
the buttons of =overblock-pydoc-buttons= — and a rule below it.  A doc
string of one line takes one row instead: the glyph, the prose, the
buttons at the window's edge, and the bar's own rule over it.  Two bars would make three rows out of
