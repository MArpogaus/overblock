...
| =overblock-fringe=        | whether a block draws a bracket in the fringe    |
...
| =overblock-fringe-face=   | the face of the bracket in the fringe            |

The options named =overblock-= belong to the block layer, and every
caller of it shares them.  The three that moved there answer to their
old names as well — =pycell-max-image-height=, =pycell-markdown-command=
and the =pycell-md-code= face — so a configuration that sets one keeps
working; it will say it is obsolete.
...
buffer with a header and a bracket in the fringe,
=overblock-md.el= turns markdown into a string such a block can show,
