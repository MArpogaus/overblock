...
- A converter from Markdown to HTML for the markdown cells.  pycell
  looks for =pandoc --mathjax --no-highlight=, =markdown_py -x tables -x
  fenced_code=, =cmark-gfm -e table=, =markdown= and =cmark=, in this
  order — the ones that can carry a table and a code fence first, each
  told to leave the math alone and to paint no code.  See
