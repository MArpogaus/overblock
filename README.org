...
=overblock-pydoc-modes= the =fontify= renderer paints with and
=overblock-pydoc-edit= opens.  reStructuredText is the default, because
...
name a =markdown-mode= derivative that hides its markup if you prefer a
quieter rendering.

=overblock-pydoc-renderer= picks between the two renderers.
=converter= hands the text to the command for the markup; =fontify=
runs the mode for the markup over the text and keeps what its font
lock painted: no process, and no line below moves.
