...
One idea, four packages: a *block* shows text over a region of a
...
Install it on its own only to write such a caller; the three packages
below pull it in.
...
- For anything that renders markup — =overblock-md= and the two
...
And for the notebook alone:

- A result block shows =overblock-pycell-max-lines= of the output and
  no more.  =overblock-pycell-pop-output= shows the whole of it, and
  keeps up with a cell that is still printing.
- One cell runs at a time for each inferior process.
  =overblock-pycell= refuses a cell while another cell runs.
...
- package-lint runs once for each package, because four live in this
  ...
  file.  The four file lists at the top of the =lint= target are the
...
|---------------------------+-----------------------------------------------------------------------------------------|
| [[file:docs/overblock.org][docs/overblock.org]]        | the block layer, the bars, the live cycle, the edit buffer, the shell output, scrolling |
