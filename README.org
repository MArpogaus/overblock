...
The closing fence carries a rule instead: three backquotes under a
result read as litter, and a chunk that opened with a bar had nothing
to close it.  =overblock-rmd-footer= is the face that draws it.
...
| =overblock-rmd-footer=          | =shadow= with an overline | the rule over the closing fence                  |
