...
chunk runs with a PNG device open, at =overblock-rmd-figure-size= —
knitr's =fig.width= and =fig.height=, 7 by 5 inches by default — and
...
- A figure of R is drawn at =overblock-rmd-figure-size=, one size for
  the file: the =fig.width= a chunk writes in its own header is not
  read.
